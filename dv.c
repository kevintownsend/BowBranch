...
/* Return the index into (*DV)->entries[] at which the "document
  ...
  static inline void grow_if_necessary (bow_dv **dv, int error_on_creation)
    {
  ...
  static inline void initialize_dv_index (bow_dv **dv, int di, int dvi)
    {
      (*dv)->entry[dvi].di = di;
  ...
  if ((*dv)->length == 0)
  ...
  else if (di > (*dv)->entry[dv_index].di)
    {
     ...
     the end of the current DV. */
      dv_index = (*dv)->length;
      ((*dv)->length)++;
      grow_if_necessary (dv, error_on_creation);
      initialize_dv_index (dv, di, dv_index);
      return dv_index;
    }
  else
    {
